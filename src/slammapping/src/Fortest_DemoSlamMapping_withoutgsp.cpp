...
<<<<<<< HEAD
    FinishStatePublisher_ = node_.advertise<std_msgs::Bool>("/slam/finishstate",1,true);
=======
>>>>>>> ff1b3416472d3deea21f69eefd306ff96578adad
    ...
    //订阅IMU数据和速度、加速度数据，消息类型未知，等于获得当前位姿
    PathByRTK_ = node_.subscribe("/livox_odometry_mapped", 1, &SlamMapping::odomCallback, this);
...
<<<<<<< HEAD
    finishstate.data = checkFinishOrNot(odom_pose);//"data=1" is finished, "data=0" is not
=======
>>>>>>> ff1b3416472d3deea21f69eefd306ff96578adad
    if(lapCount_.data ==1){
        laser_number_now = 0;
...
<<<<<<< HEAD
bool SlamMapping::checkFinishOrNot(const Mapping::OrientedPoint &mpose){
    ROS_INFO("we are now at %lf", mpose.x);
    if(mpose.x>=75){
        return true;
    }
    else{
...
/*里程计信息的回调函数,using IMU*/
void SlamMapping::odomCallback(const nav_msgs::Odometry &modom)
{
    odom_number_ ++;
    ROS_INFO("We are now at %d frame", odom_number_);
    modom_ = modom;
...
/*ODOM数据处理与转化*/
void SlamMapping::odomProcess(const nav_msgs::Odometry &modom){
    odom_pose.x = modom.pose.pose.position.x;
    odom_pose.y = modom.pose.pose.position.y;
    //设置三个欧拉角，roll, pitch, yaw
    geometry_msgs::Vector3 mEulerAngles_;
    geometry_msgs::Quaternion mQuater_;
    mQuater_.x = modom.pose.pose.orientation.x;
    mQuater_.y = modom.pose.pose.orientation.y;
    mQuater_.z = modom.pose.pose.orientation.z;
    mQuater_.w = modom.pose.pose.orientation.w;

    //四元数转欧拉角
    QuaterToEuler(mQuater_, mEulerAngles_);
    odom_pose.theta = mEulerAngles_.z;
    ...
    carstate_.car_state.y = odom_pose.y;
    carstate_.car_state.theta = odom_pose.theta;
    ROS_INFO("Odom Setting Successful");
}

/*四元数转欧拉角*/
void SlamMapping::QuaterToEuler(const geometry_msgs::Quaternion &mQuater, geometry_msgs::Vector3 &mEulerAngles){
    double x,y,z,w;
...
void SlamMapping::AccCallback(const geometry_msgs::Vector3::ConstPtr& macc){
    //accleration at Map
    carstate_.car_state_dt.car_state_a.x = macc->x;
    carstate_.car_state_dt.car_state_a.y = macc->y;
    carstate_.car_state_dt.car_state_a.theta = macc->z;
...
<<<<<<< HEAD
    FinishStatePublisher_.publish(finishstate);
