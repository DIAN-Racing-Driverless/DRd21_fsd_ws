...
visualization_msgs::MarkerArray Control::getMapPath() {return mapPath_;}
...
void Control::setTrack() {
  ...
  if (!is_init) {
    ...
    map_traj_.clear();
    mapPath_.markers.clear();

    track_->genTraj();
    map_traj_ = track_->getMapTraj();
    std::vector<float> color_path = {0, 0, 1};
    visual_trajectory(map_traj_, mapPath_, "/liox_frame", color_path,
                  car_state_.header, true);  
    ROS_INFO("map path size is %ld",mapPath_.markers.size());
...
void Control::runAlgorithm() {
  ...
  if(cmd_.steering_angle.data<-1)
        cmd_.steering_angle.data=-1;
  if(cmd_.steering_angle.data>1)
        cmd_.steering_angle.data=1;

  if(cmd_.throttle.data>1)
        cmd_.throttle.data=1;  
  ...
  visual_trajectory(map_traj_, mapPath_, "/livox_frame", color_path,
                  car_state_.header, true);  
  ROS_INFO("map path size is %ld",mapPath_.markers.size());
